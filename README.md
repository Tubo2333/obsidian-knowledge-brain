...
每次和 AI 编程解决了 bug、做了技术决策，下次对话就忘了。**memory-brain** 帮你记住——自动构建可检索的知识库，持续进化项目规则。一个从每次对话中学习的项目图书管理员。
...
```bash
git clone https://github.com/Tubo2333/obsidian-knowledge-brain.git .claude/skills/memory-brain/
```
...
Copy the `memory-brain/` folder into your platform's skill directory. / 把 `memory-brain/` 复制到对应平台目录：
